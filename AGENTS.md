...
`npm install -g @asc-agent/runtime@0.8.1`, then re-run the same subcommand as `asc …`.
...
```bash
asc profile adopt --json     # before install: npx --yes @asc-agent/bootstrap@0.8.1 profile adopt --json
```
