...
`npm install -g @asc-agent/runtime@0.5.2`, then re-run the same subcommand as `asc …`.
...
```bash
asc profile adopt --json     # before install: npx --yes @asc-agent/bootstrap@0.5.2 profile adopt --json
```
