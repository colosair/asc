...
npx --yes @asc-agent/bootstrap@0.10.1 setup apply --json
        ...
        ↓  apply: npm install -g @asc-agent/runtime@0.10.1
...
| 방식 | 진입 | 언제 |
|---|---|---|
| Zero-install | `npx --yes @asc-agent/bootstrap@0.10.1 <command> --json` | 아직 아무것도 설치 전 |
| Persistent | `asc setup` 이 설치하고 `asc update` 가 옮긴다 | 안정적인 로컬 명령. `npm install -g @asc-agent/runtime@0.10.1` 은 `npx` 자체가 서지 못할 때의 수동 대안이다 |
