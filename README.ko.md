...
npx --yes @asc-agent/bootstrap@0.5.0 init
        ...
        ↓  apply: npm install -g @asc-agent/runtime@0.5.0
...
| 방식 | 진입 | 언제 |
|---|---|---|
| Zero-install | `npx --yes @asc-agent/bootstrap@0.5.0 <command> --json` | 아직 아무것도 설치 전 |
| Persistent | `npm install -g @asc-agent/runtime@0.5.0` 후 `asc <command>` | 안정적인 로컬 명령 — 그리고 `npx` 자체가 서지 못하는 기계의 fallback |
