...
```bash
npx --yes @asc-agent/bootstrap@0.7.0 init
```
...
npx --yes @asc-agent/bootstrap@0.7.0 init
        ...
        ↓  apply runs: npm install -g @asc-agent/runtime@0.7.0
...
| Tier | Entry | When |
|---|---|---|
| Zero-install | `npx --yes @asc-agent/bootstrap@0.7.0 <command> --json` | nothing is installed yet |
| Persistent | `npm install -g @asc-agent/runtime@0.7.0`, then `asc <command>` | the stable local command — and the fallback when `npx` itself cannot start |
...
```jsonc
{
  ...
  "changes": [
    { "target": "runtime-install", "package": "@asc-agent/runtime",
      "version": "0.7.0", "strategy": "npm-global", "from": "NOT_INSTALLED" },
  ...
  "actions": [
    ...
    { "type": "apply_setup",
      ...
      "portable": "npx --yes @asc-agent/bootstrap@0.7.0 setup apply" }
  ],
  "nextActions": ["npx --yes @asc-agent/bootstrap@0.7.0 setup apply"],
