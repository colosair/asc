...
npx --yes @asc-agent/bootstrap@0.3.0 init
        ...
        ↓  apply runs: npm install -g @asc-agent/runtime@0.3.0
...
| Tier | Entry | When |
|---|---|---|
| Zero-install | `npx --yes @asc-agent/bootstrap@0.3.0 <command> --json` | nothing is installed yet |
| Persistent | `npm install -g @asc-agent/runtime@0.3.0`, then `asc <command>` | the stable local command — and the fallback when `npx` itself cannot start |
...
```jsonc
{
  ...
  "actions": [
    ...
    { "type": "apply_setup",
      ...
      "portable": "npx --yes @asc-agent/bootstrap@0.3.0 setup apply" }
  ],
  "nextActions": ["npx --yes @asc-agent/bootstrap@0.3.0 setup apply"],
