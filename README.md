...
npx --yes @asc-agent/bootstrap@0.10.1 setup apply --json
        ...
        ↓  apply runs: npm install -g @asc-agent/runtime@0.10.1
...
| Tier | Entry | When |
|---|---|---|
| Zero-install | `npx --yes @asc-agent/bootstrap@0.10.1 <command> --json` | nothing is installed yet |
| Persistent | `asc setup` installs it; `asc update` moves it | the stable local command. `npm install -g @asc-agent/runtime@0.10.1` is the manual fallback for when `npx` itself cannot start |
...
```jsonc
{
  ...
  "changes": [
    { "target": "runtime-install", "package": "@asc-agent/runtime",
      "version": "0.10.1", "strategy": "npm-global", "from": "NOT_INSTALLED" },
  ...
  "actions": [
    ...
    { "type": "apply_setup",
      ...
      "portable": "npx --yes @asc-agent/bootstrap@0.10.1 setup apply" }
  ],
  "nextActions": ["npx --yes @asc-agent/bootstrap@0.10.1 setup apply"],
