...
> **HISTORICAL DESIGN SNAPSHOT — v5.1 (2026-08-22 동결).**
>
> 이 문서는 **현재 제품의 정본이 아니다.** 2026-08-22 에 동결된 설계 기준을 그대로
> 보존하며, 코드·문서 곳곳의 `OM §x` 참조가 가리키는 자리다(현재 225곳). 그래서 번호와
> 경로를 바꾸지 않고, 본문 문장도 현재 시제로 고쳐 쓰지 않는다 — 과거 결정을 오늘의
> 말로 덮으면 그 시점에 무엇을 근거로 정했는지 아무도 되짚을 수 없다.
>
> ```text
> 현재 제품 모델    docs/design/current-operating-model.md
> 현재 구현 상태    docs/status.md
> 현재 사용자 흐름  README.md
> ```
>
> 아래 본문에는 그 뒤 실제로 달라진 것들이 남아 있다. 몇 가지는 이름을 대는 편이
> 정확하다: 이 문서가 "아직 구현하지 않는다" 고 적은 Core·CLI·Adapter·Runtime·Profile
> schema 는 그 뒤 전부 구현돼 게시됐고, Queue 는 0.7.0 에서 삭제됐으며, Execution Mode ·
> Remote Review · read-back verify 는 0.8.0 에서 들어왔다.
>
> 작성: 2026-08-22 (v4: 2026-08-21 / v5: 2026-08-22). 변경 이력은 부록 A(Change Summary)에만
> 요약한다. 본문 YAML·파일명·상태값은 책임 경계 검증에 필요한 수준의 예시이며 구현
> 확정이 아니다.
