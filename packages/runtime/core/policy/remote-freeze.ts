...
//
// 차단이 서는 자리는 ASC 가 스스로 수행하는 것뿐이다: 관리 실행기(Executor, `remote.write`)와
// ASC 자신의 원격 읽기(monitor, `remote.read`). 같은 OS 사용자가 셸에서 직접 치는 raw 명령은
// ASC 의 경계 밖이다 — 0.9.0 에서 그 경계를 가장하던 hook 을 걷어냈다.
//
