...
describe('F6 — 일이 시작되면 논리 세션 안에서 시작된다', () => {
  ...
  it('0.9.0 — PreToolUse 에는 아무것도 심지 않는다. hook 은 SessionStart 하나다', () => {
    const specs = locate({ claudeHome: '/home/me/.claude', entry: '/opt/asc/asc.js' }).hooks
    assert.deepEqual(specs.map((hook) => hook.event), ['SessionStart'])
    assert.deepEqual(locate({ claudeHome: '/home/me/.claude' }).hooks, [])
  })
})
