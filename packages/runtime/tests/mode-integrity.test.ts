...
//
//   기록이 없다   아무도 고르지 않았다        → 사람이 실행한다 (ADVISE)
//   못 읽는다     무엇이 저장돼 있었는지 모른다 → 관리 실행기는 AUTO 로 다룬다 (ENFORCE, read-back 필수)
//
...
describe('0.8.4 — Execution Mode 는 Run 이 답한다', () => {
  ...
  it('한 Run 이 내려가도 다른 Run 의 강제는 그대로다', () => {
    const record = {
      mode: 'AUTO' as const,
      since: NOW,
      runs: { 'run-a': { mode: 'MANUAL' as const, since: NOW, by: 'colosair', reason: 'acceptance' } },
    }
    assert.equal(enforcementOf(resolveExecutionMode(record, 'run-a')), 'ADVISE', '자기 답이 MANUAL 인 Run 은 사람이 실행한다')
    assert.equal(enforcementOf(resolveExecutionMode(record, 'run-b')), 'ENFORCE', '다른 Run 은 workspace 의 AUTO 그대로다')
  })
